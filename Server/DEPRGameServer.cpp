...
//
// GameServer::GameServer(const SaveManager &db) : db(db), server(Server()) {
...
//         const auto players = this->db.LoadWhere(LOOKUP_TABLE(Players), {}, COL(Players, score),
//                                                 Config::Server::LeaderboardSize);
//         json j = json::array();
//         for (const auto p: players)
//             j.push_back({
//                 {COL(Players, ID), p[COL(Players, ID)]},
...
//     PlayerSession playerID = PlayerSession(2, "TODO");
//     if (true) {
//         res.status = 401;
